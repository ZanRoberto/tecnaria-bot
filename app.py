...
function generateOfferta() {
  if (selected.length === 0) { alert('Seleziona almeno 1 brand'); return; }
  const q = 'Genera una PROPOSTA COMMERCIALE COMPLETA per il cliente che vuole acquistare prodotti di: ' + selected.join(', ') + '. Includi: prezzi competitivi, caratteristiche principali, vantaggi rispetto ai competitor, garanzie, tempi di consegna, modalità di pagamento, supporto post-vendita e consigli di installazione.';
...
function generateAnalisi() {
  if (selected.length === 0) { alert('Seleziona almeno 1 brand'); return; }
  const q = 'Fai una ANALISI APPROFONDITA DEL MERCATO per i brand: ' + selected.join(', ') + '. Analizza: posizionamento di mercato, competitor principali, trend attuali, punti di forza e debolezza, opportunità di crescita, rischi, prezzi di mercato, customer preferences, e strategie vincenti nel settore dell\'arredo bagno.';
  document.getElementById('question').value = q;
  ask();
}
...
function generateProposta() {
  if (selected.length === 0) { alert('Seleziona almeno 1 brand'); return; }
  const q = 'Crea una PROPOSTA STRATEGICA OPERATIVA per: ' + selected.join(', ') + '. Includi: piano d\'azione dettagliato, target di vendita, strategie di marketing, partnership suggerite, investimenti necessari, timeline di implementazione, KPI da monitorare, budget indicativo, e previsioni di ROI.';
  document.getElementById('question').value = q;
  ask();
}
