...
/* ========== V10 LAYOUT 3-COLONNE ========== */
.container-main { display: flex; width: 100%; height: 100vh; }
.sidebar { width: 22%; background: #0a0f1f; border-right: 1px solid #334155; overflow-y: auto; }
.center { width: 48%; background: #0f172e; overflow-y: auto; padding: 20px; }
.drawer-piani { position: fixed; right: 0; top: 0; width: 30%; height: 100vh; background: #1a2744; border-left: 2px solid #3b82f6; transform: translateX(100%); transition: transform 0.3s; z-index: 1000; overflow-y: auto; }
.drawer-piani.open { transform: translateX(0); }

/* CARD PRODUCT */
.product-card { background: #1e2d4a; border: 1px solid #334155; border-radius: 8px; padding: 12px; margin-bottom: 12px; display: flex; gap: 10px; }
.product-card-img { width: 80px; height: 80px; background: #0a0f1f; border-radius: 4px; overflow: hidden; flex-shrink: 0; }
.product-card-img img { width: 100%; height: 100%; object-fit: cover; }
.product-card-info { flex: 1; font-size: 11px; }
.product-card-brand { color: #60a5fa; font-weight: bold; }
.product-card-price { color: #10b981; font-weight: bold; font-size: 13px; margin: 4px 0; }
.product-card-actions { display: flex; gap: 6px; }

/* DRAWER HEADER */
.drawer-header-v10 { background: #2a4a7f; border-bottom: 1px solid #334155; padding: 15px; display: flex; justify-content: space-between; align-items: center; }

/* PIANO/STANZA/VOCE in drawer */
.piano-item { background: #1e3a5f; border-left: 4px solid #3b82f6; margin: 8px 0; padding: 10px; cursor: pointer; border-radius: 4px; }
.piano-item-header { display: flex; justify-content: space-between; align-items: center; color: #60a5fa; font-weight: bold; }
.stanza-item { background: #1e293b; border-left: 3px solid #8b5cf6; margin: 6px 0 6px 12px; padding: 8px; cursor: pointer; border-radius: 3px; }
.stanza-item-header { display: flex; justify-content: space-between; align-items: center; color: #c4b5fd; font-weight: 600; font-size: 12px; }
.voce-item-drawer { background: #0f1729; border-left: 2px solid #6b7280; padding: 6px; margin: 4px 0 4px 12px; border-radius: 3px; font-size: 10px; display: flex; justify-content: space-between; align-items: center; }
.voce-brand { color: #60a5fa; font-weight: bold; }
.voce-price { color: #10b981; font-weight: bold; }
...
// ========== V10 DRAWER CONTROLLER ==========
let drawerOpen = false;
let cantiereAttivo = null;

function apriDrawer(cantiere_id) {
    cantiereAttivo = cantiere_id;
    document.getElementById('drawer-piani').classList.add('open');
    drawerOpen = true;
    caricaStrutturaPiani(cantiere_id);
}
...
function caricaStrutturaPiani(cantiere_id) {
    fetch(`/api/cantieri/${cantiere_id}/struttura`)
        .then(r => r.json())
        .then(d => {
            if (!d.ok) {
                console.error('Errore caricamento struttura:', d);
                return;
            }
            renderStrutturaPiani(d.piani, d.cantiere, d.totale_generale || 0);
        })
        .catch(e => console.error('Fetch error:', e));
}

function renderStrutturaPiani(piani, cantiere, totaleGenerale) {
    let html = `<div class="drawer-header-v10">
        <h2 style="margin: 0; color: #60a5fa; font-size: 16px;">${cantiere[1]}</h2>
        <button onclick="chiudiDrawer()" style="background: #6b7280; border: none; color: white; padding: 6px 10px; cursor: pointer; border-radius: 4px;">✕</button>
    </div><div style="padding: 15px;">`;
    
    piani.forEach(p => {
        html += `
        <div class="piano-item">
            <div class="piano-item-header" onclick="togglePiano(this)">
                <span style="font-size: 11px; margin-right: 5px;">▼</span>
                <strong>📍 ${p.nome}</strong>
                <span style="color: #10b981; font-weight: bold;">€${p.totale.toFixed(2)}</span>
            </div>
            <div class="piano-content" style="display: block;">
        `;
        
        p.stanze.forEach(s => {
            html += `
            <div class="stanza-item">
                <div class="stanza-item-header" onclick="toggleStanza(this)">
                    <span style="font-size: 10px; margin-right: 5px;">▼</span>
                    <strong>🚿 ${s.nome}</strong>
                    <span style="color: #10b981; font-weight: bold;">€${s.totale.toFixed(2)}</span>
                </div>
                <div class="stanza-content" style="display: block;">
            `;
            
            s.voci.forEach(v => {
                html += `
                <div class="voce-item-drawer">
                    <div>
                        <div class="voce-brand">${v.brand} [${v.codice || '—'}]</div>
                        <div style="color: #d1d5db; font-size: 9px;">${v.descrizione}</div>
                        <div style="color: #9ca3af; font-size: 9px;">${v.quantita}x €${v.prezzo.toFixed(2)}</div>
                    </div>
                    <div class="voce-price">€${v.subtotale.toFixed(2)}</div>
                    <div style="display: flex; gap: 4px;">
                        <button onclick="modificaVoceModal(${v.id})" style="background: #6b7280; border: none; color: white; padding: 3px 6px; cursor: pointer; border-radius: 3px; font-size: 10px;">✏️</button>
                        <button onclick="eliminaVoceConfirm(${v.id})" style="background: #ef4444; border: none; color: white; padding: 3px 6px; cursor: pointer; border-radius: 3px; font-size: 10px;">✕</button>
                    </div>
                </div>
                `;
            });
            ...
            html += `
                ...
                <button onclick="aggiungiVoceModal(${s.id})" style="width: 100%; margin-top: 6px; background: rgba(16,185,129,0.2); color: #6ee7b7; border: none; padding: 6px; cursor: pointer; border-radius: 3px; font-size: 10px;">➕ Voce</button>
            </div>
            `;
        });
        ...
        html += `
                ...
                <button onclick="aggiungiStanzaModal(${p.id})" style="width: 100%; margin-top: 8px; background: rgba(139,92,246,0.3); color: #a78bfa; border: none; padding: 6px; cursor: pointer; border-radius: 3px; font-size: 11px;">➕ Stanza</button>
            </div>
        `;
    });
    
    html += `
        <button onclick="aggiungiPianoModal(${cantiereAttivo})" style="width: 100%; margin-top: 12px; background: rgba(59,130,245,0.3); color: #93c5fd; border: none; padding: 8px; cursor: pointer; border-radius: 3px; font-weight: bold;">➕ Piano</button>
        <div style="background: rgba(16,185,129,0.2); border: 1px solid rgba(16,185,129,0.5); padding: 12px; margin-top: 15px; border-radius: 6px; text-align: center;">
            <div style="color: #9ca3af; font-size: 11px;">TOTALE GENERALE</div>
            <div style="font-size: 18px; font-weight: bold; color: #10b981;">€${totaleGenerale.toFixed(2)}</div>
        </div>
    </div>`;
    
    document.getElementById('drawer-piani').innerHTML = html;
}

function togglePiano(el) {
    const content = el.closest('.piano-item').querySelector('.piano-content');
    content.style.display = content.style.display === 'none' ? 'block' : 'none';
}

function toggleStanza(el) {
    const content = el.closest('.stanza-item').querySelector('.stanza-content');
    content.style.display = content.style.display === 'none' ? 'block' : 'none';
}

function aggiungiPianoModal(cid) {
    const nome = prompt('Nome piano:');
    if (!nome) return;
    fetch(`/api/cantieri/${cid}/piani`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({nome})
    }).then(r => r.json()).then(d => {
        if (d.ok) caricaStrutturaPiani(cid);
    });
}

function aggiungiStanzaModal(pid) {
    const nome = prompt('Nome stanza:');
    if (!nome) return;
    fetch(`/api/piani/${pid}/stanze`, {
        ...
        if (d.ok) caricaStrutturaPiani(cantiereAttivo);
    });
}
...
function aggiungiVoceModal(sid) {
    ...
    if (!brand || !desc || isNaN(qty) || isNaN(prezzo)) return;
...
function eliminaVoceConfirm(vid) {
    if (!confirm('Eliminare questa voce?')) return;
    fetch(`/api/stanza_voci/${vid}`, {method: 'DELETE'})
        .then(r => r.json())
        .then(d => {
            if (d.ok) caricaStrutturaPiani(cantiereAttivo);
        });
...
// ========== ADD TO STANZA (dal listino center) ==========
function aggiungiAlCarrelloModal(prodotto) {
    // Modale per scegliere la stanza
    if (!cantiereAttivo) {
        alert('Seleziona un cantiere dal drawer');
        return;
    }
    ...
    fetch(`/api/cantieri/${cantiereAttivo}/struttura`)
        ...
        .then(d => {
            ...
            d.piani.forEach(p => {
                p.stanze.forEach(s => {
                    stanze.push({id: s.id, nome: `${p.nome} → ${s.nome}`, piano_id: p.id});
                });
            });
            
            if (stanze.length === 0) {
                alert('Aggiungi almeno una stanza nel cantiere');
                return;
            }
            
            let opzioni = stanze.map((s, i) => `${i+1}. ${s.nome}`).join('\n');
            let scelta = prompt('Scegli stanza:\n' + opzioni);
            
            if (!scelta) return;
            let idx = parseInt(scelta) - 1;
            if (idx < 0 || idx >= stanze.length) return;
            
            let stanza_id = stanze[idx].id;
            
            // Aggiungi voce
            fetch(`/api/stanze/${stanza_id}/voci`, {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({
                    ...
                    quantita: 1,
                    prezzo_unitario: prodotto.prezzo_cliente || prodotto.prezzo || 0
                })
            }).then(r => r.json()).then(d => {
                if (d.ok) {
                    alert('✅ Aggiunto a stanza!');
                    caricaStrutturaPiani(cantiereAttivo);
                }
            });
        });
}
...
<!-- DRAWER PIANI/STANZE/VOCI V10 -->
<div id="drawer-piani" class="drawer-piani"></div>
...
# ============================================================================
# API ENDPOINTS - PIANI/STANZE/VOCI (V10 INTEGRATION)
# ============================================================================
